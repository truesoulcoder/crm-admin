...
// POST handler to start a campaign
export async function POST(
  ...
  try {
    ...
    console.error('API POST /engine/campaigns/[campaign_id]/start error:', error);
    const errorMessage = error instanceof Error ? error.message : String(error);
    return NextResponse.json(
      { success: false, error: `Failed to start campaign: ${errorMessage}` },
      { status: 500 }
    );
  }
}
...
export async function GET(
  ...
  try {
    ...
    if (!campaign_id) {
      // This case should ideally not be hit if Next.js calls this for a dynamic segment
      return NextResponse.json(
    ...
    console.error('API GET /engine/campaigns/[campaign_id]/start error:', error);
    const errorMessage = error instanceof Error ? error.message : String(error);
    return NextResponse.json(
